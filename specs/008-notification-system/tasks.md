...
- [x] T000 Scaffold `@hakwa/notifications` workspace package — create
      ...
      `api/` before any Phase 2 task. (Satisfies Constitution I — Package-First)
- [x] T001 Define `NotificationType` enum (16 values: `booking_confirmed`,
...
- [x] T002 Define `NotificationChannel` enum (`push | in_app | email | sms`) and
      `NotificationStatus` enum (`pending | sent | failed`) and `DevicePlatform`
      enum (`ios | android`) in `pkg/db/schema/notification.ts`
- [x] T003 Define `notification` table schema (id, userId FK→user, type,
...
- [x] T007 Implement `pkg/notifications/src/expoPush.ts` —
...
- [x] T008 Implement `pkg/notifications/src/index.ts` — export
      ...
      `sendSms` wrappers
- [x] T009 Implement `pkg/notifications/src/worker.ts` —
...
- [x] T010 Register `pkg/notifications/src/worker.ts` as a `@hakwa/workers` pool
      job in `api/src/index.ts`; start on server boot
- [x] T011 Subscribe to `user:{userId}:notifications` Redis pub/sub in
      `api/src/websocket.ts` — relay incoming in-app notification events to the
      user's active WebSocket connection
- [x] T011b **[FR-015]** Add structured dispatch logging in
      `pkg/notifications/src/worker.ts` and `pkg/notifications/src/index.ts`: on
      every `sendNotification()` call log
      `{ event: 'notification.queued',     userId, type, channel }`; in worker
      on delivery success log
      `{ event:     'notification.sent', notificationId, userId, channel, status: 'sent' }`;
      on `DeviceNotRegistered` warn
      `{ event: 'device.deactivated', pushToken,     userId }`; on delivery
      failure error
      `{ event: 'notification.failed',     notificationId, userId, channel, errorDetail, retryCount }`;
      on Redis subscription lifecycle in `websocket.ts` log
      `{ event:     'ws.subscribed'|'ws.unsubscribed', userId }`. (Satisfies
      FR-015 and Constitution V structured logging requirement)
- [x] T037 **[Constitution II — Type Safety]** Define Zod validation schemas in
      `pkg/notifications/src/types.ts` for all API boundaries: (1)
      `DeviceRegistrationSchema` — validates `pushToken` format + `platform`
      enum; (2) `SendNotificationSchema` — validates `userId`, `type` (against
      `NotificationType` enum), `channel`, `title`, `body`, `data` shape. Apply
      schemas at route handler boundaries (`POST /api/devices`,
      `PATCH /api/me/notification-preferences/:type/:channel`) and inside
      `sendNotification()` so invalid payloads return `400` before reaching
      service logic. Mark Principle II satisfied in plan.md.
...
- [x] T012 [US1] Add `sendNotification` calls at each trip status transition in
...
- [x] T013 [P] [US1] Add `sendNotification` for `receipt_generated` type in
      `api/src/services/tripService.ts` after fare split commits
- [x] T014 [P] [US1] Add `sendNotification` for `wallet_credited` type in
      `pkg/notifications` after merchant wallet ledger insert in
      `api/src/services/walletService.ts`
- [x] T015 [P] [US1] Add `sendNotification` for `payout_processed` and
...
- [x] T016 [US2] Implement `POST /api/devices` in `api/src/routes/devices.ts` —
...
- [x] T021 [US3] Seed `notificationPreference` rows (16 types × 4 channels = 64
      rows) on user registration in `pkg/auth/lib/auth.ts` inside the
      `onUserCreated` hook
- [x] T022 [US3] Add preference check in `sendNotification()` in
      ...
      (always dispatch)
- [x] T023 [P] [US3] Implement `GET /api/me/notification-preferences` in
...
- [x] T026 [US4] Implement `GET /api/notifications` in
...
- [x] T027 [US4] Implement `PATCH /api/notifications/:id/read` in
      `api/src/routes/notifications.ts` — session required; verify ownership;
      set `readAt = now()` if null
- [x] T028 [P] [US4] Implement `POST /api/notifications/read-all` in
...
- [x] T029 [P] [US4] Maintain Redis unread counter — `INCR user:{userId}:unread`
...
- [x] T032 [US5] Implement daily re-engagement job in
...
- [x] T033 [US5] Schedule re-engagement job at 09:00 Fiji time daily using
...
- [x] T034 [P] Add `sendNotification` calls for gamification events
      (`badge_earned`, `level_up`, `streak_milestone`, `referral_conversion`) in
      `pkg/workers/src/workers/gamificationProcessor.ts`
- [x] T035 [P] Implement EPN receipt poller in
      `pkg/notifications/src/expoPush.ts` — after 15 s delay call
      `Expo.getPushNotificationReceiptsAsync`; for `DeviceNotRegistered`
      receipts `UPDATE device SET active = false` (permanently dead — do NOT
      re-activate); for `error` receipts implement exponential back-off: compute
      delay as `Math.min(2 ** retryCount * 1000, 60_000)` ms (capped at 60 s),
      then re-queue via
      `setTimeout(() => redis.xadd('notifications:outbox',     '*', { ...payload, retryCount: retryCount + 1 }), delay)`
      if `retryCount < MAX_RETRY_COUNT` (constant = 5 in
