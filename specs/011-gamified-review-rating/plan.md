...
Points are awarded atomically within the review submission transaction (10
rating-only, 15 with 2+ tags, 25 with 2+ tags + comment) using the
...
| Reputation score            | `AVG(rating)` across revealed reviews computed at query time; Redis cache `reputation:{userId}` TTL 5min            |
...
| --------------------------- | ------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| I. Auth required            | FR-001 | [x]    | All `/api/v1/reviews/*` endpoints use `requireAuth`; `GET /reviews/user/:userId` is also authenticated                                           |
...
| VII. Gamification           | FR-007 | [x]    | Points awarded in-transaction (10/15/25); 8 new badge triggers via gamification worker; Redis leaderboard updated via existing Spec 007 pipeline |
...
| IX. Cursor pagination       | FR-009 | [x]    | `GET /reviews/trip/:tripId` is non-paginated (at most 2 records); history-style responses paginate by `submittedAt` cursor where applicable      |
...
| XVIII. Official Docs First  | FR-018 | [x]    | Drizzle unique-constraint syntax, Redis `XADD` stream publish, and notification dispatch/queue patterns verified against official docs.          |
...
api/
  src/
    routes/
      reviews.ts         ← GET /reviews/tags, POST /reviews, GET /reviews/trip/:tripId, GET /reviews/me, GET /reviews/user/:userId, GET /reviews/passenger-signal/:userId, GET /reviews/driver-signal/:userId
    services/
