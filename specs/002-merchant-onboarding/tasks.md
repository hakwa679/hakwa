...
- [x] T001 Add `userId`, `licenseType`, `status`, `nationalId`, `phone` columns
      to `merchant` table in `pkg/db/schema/merchant.ts`
- [x] T002 Create `vehicle` table (`id`, `merchantId`, `make`, `model`, `year`,
      `registrationPlate`, `seatingCapacity`, `color`, `isActive`, `createdAt`,
      `updatedAt`) in `pkg/db/schema/merchant.ts`
- [x] T003 Export updated `merchant` and new `vehicle` schema from
      `pkg/db/schema/index.ts`
- [x] T004 Run `db-push` to apply schema and confirm `merchant` columns and
      `vehicle` table exist
...
- [x] T020 [US2] Update `BusinessDetailsScreen.tsx` to conditionally show
      `nationalId` field and hide TIN / LTA fields when
      `licenseType = 'unlicensed'`
- [x] T021 [US2] Update `PATCH /api/merchants/me` validation in
      `api/src/routes/merchants.ts` — require `nationalId` for unlicensed;
      require TIN + businessRegistrationNumber for licensed
- [x] T022 [US2] Add duplicate TIN check in `merchantService.ts` — reject
...
- [x] T023 [US3] Implement `PUT /api/merchants/me/bank-account` in
      ...
      merchants
- [x] T024 [US3] Build "Edit Payout Account" screen in
      `apps/mobile/merchant/src/screens/settings/PayoutAccountScreen.tsx` —
      shows current bank details with edit form
- [x] T025 [US3] Wire edit form to `PUT /api/merchants/me/bank-account` via
...
- [x] T031 [P] Mid-flow wizard progress persistence — save each step's data to
      local state so that on app restart the wizard resumes from the last
      incomplete step
- [x] T032 [P] Merchant dashboard status banner — show clear status message
      based on `status` field (`under_review`, `approved`, `rejected`) in
      `apps/mobile/merchant/src/screens/MerchantDashboard.tsx`
- [x] T033 [P] Admin notification dispatch on `POST /api/merchants/me/submit` —
      verify `@hakwa/notifications` Redis Stream event is published post-commit
      in `merchantService.ts`
- [x] T034 [P] Validate all merchant API routes return correct `AppError`
