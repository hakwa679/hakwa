...
**Notes**:
- `feature_id` is nullable so the log is not destroyed if a feature is
  later hard-deleted.
...
Flags raised by the nightly abuse-detection job (FR-039). Maintained
via upsert to prevent table bloat on repeat detections.
...
**Constraints**:
- `UNIQUE (user_id, flag_type)` — upsert target; one flag record per user
  per type, with `occurrence_count` tracking repeat detections.
